  ...
  const onSubmit = async (data: SignupInput) => {
    ...
    if (auth.user) {
      const { error: pe } = await sb.from("users").insert({
        id: auth.user.id, email: data.email, full_name: data.full_name, phone: data.phone, role,
      });
      if (pe && !pe.message.includes("duplicate")) { setErr(pe.message); return; }
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-[#0a0a0a]">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <Label className="text-white/60 text-sm">Phone <span className="text-white/25">(optional)</span></Label>
            <Input type="tel" placeholder="+91 9999 999999" {...register("phone")} />
          </div>
          <div className="space-y-1.5">
            <Label className="text-white/60 text-sm">Password</Label>
            <div className="relative">
              <Input type={show ? "text" : "password"} placeholder="Min. 8 characters" error={errors.password?.message} {...register("password")} />
          ...
          <div className="space-y-1.5">
            ...
            <Input type="password" placeholder="Re-enter password" error={errors.confirm_password?.message} {...register("confirm_password")} />
          </div>
