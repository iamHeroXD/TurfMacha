...
const TYPE_CONFIG: Record<string, { label: string; icon: typeof ArrowUpRight; color: string; bg: string }> = {
  earn:         { label: "Earned",     icon: ArrowUpRight,  color: "text-[#0D4D36]", bg: "bg-[#0D4D36]/8"  },
  ...
  expire:       { label: "Expired",    icon: ArrowDownLeft, color: "text-[#9E9284]", bg: "bg-[#F4F1EB]"      },
};
...
export default function WalletPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F4F1EB] pt-24 pb-24 md:pb-8">
      {/* Header */}
      <div className="bg-white border-b border-[#E7E2DA]">
        <div className="max-w-2xl mx-auto px-4 py-6 flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-[#0D4D36] flex items-center justify-center shrink-0">
            <Coins className="h-6 w-6 text-[#A6D96A]" />
          </div>
          <div>
            <h1 className="font-display font-bold text-[#111111] text-lg">TurfCoins Wallet</h1>
            <p className="text-xs text-[#5F5F5F] mt-0.5">Earn coins on every booking</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        <motion.div
          ...
          className="rounded-3xl bg-[#0D4D36] p-6 text-center shadow-lg shadow-[#0D4D36]/20"
        >
          {loading ? <Skeleton className="h-12 w-32 mx-auto mb-2 opacity-20" /> : (
            <motion.p
              ...
              className="text-5xl font-bold text-[#A6D96A] tabular-nums"
            >
        ...
        >
          <p className="text-xs font-semibold text-[#5F5F5F] uppercase tracking-wider mb-3">How it works</p>
          <div className="space-y-2 text-sm text-[#5F5F5F]">
        ...
        >
          <p className="text-xs font-semibold text-[#5F5F5F] uppercase tracking-wider mb-3">Transaction History</p>

          {loading ? (
            <div className="space-y-2">{[...Array(5)].map((_, i) => <Skeleton key={i} className="h-14 rounded-xl" />)}</div>
          ) : transactions.length === 0 ? (
            <div className="py-12 text-center rounded-2xl border border-[#E7E2DA] bg-white shadow-sm">
              <Coins className="h-10 w-10 text-gray-200 mx-auto mb-3" />
              <p className="text-sm text-[#5F5F5F]">No transactions yet</p>
              <p className="text-xs text-[#9E9284] mt-1">Book a turf with rewards enabled to start earning</p>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-[#111111] font-medium">{tx.description || cfg.label}</p>
                      <p className="text-xs text-[#9E9284]">{format(new Date(tx.created_at), "d MMM yyyy, h:mm a")}</p>
                    </div>
                    <p className={`text-sm font-bold tabular-nums ${tx.amount > 0 ? "text-[#0D4D36]" : "text-red-500"}`}>
