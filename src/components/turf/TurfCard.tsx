  ...
  return (
    ...
    >
      <Link href={`/turfs/${turf.id}`} className="block group">
        <div className="bg-white border border-[#E7E2DA] rounded-2xl overflow-hidden transition-all duration-200 group-hover:border-[#C4BAB0] group-hover:shadow-lg group-hover:shadow-black/6">

          {/* Image */}
          <div className="relative h-48 overflow-hidden bg-[#EDE9DF]">
            <Image
            ...
            <div className="absolute top-3 left-3 flex flex-col gap-1">
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-white/90 text-[#111111] backdrop-blur-sm shadow-sm">
            ...
            <div className="absolute bottom-3 right-3">
              <span className="inline-flex items-baseline gap-0.5 px-2.5 py-1 rounded-full text-sm font-bold bg-white/90 text-[#111111] backdrop-blur-sm shadow-sm">
          ...
          <div className="p-4">
            <div className="flex items-start justify-between gap-2 mb-1.5">
              <h3 className="font-semibold text-[#111111] text-sm leading-snug group-hover:text-[#0D4D36] transition-colors duration-150 line-clamp-1">
                {turf.name}
              </h3>
              {turf.rating !== undefined && turf.rating !== null && (
                <div className="flex items-center gap-1 shrink-0 text-xs">
                  <Star className="h-3 w-3 star-filled shrink-0" />
                  <span className="text-[#111111] font-semibold">{turf.rating.toFixed(1)}</span>
            ...
            {(turf.sports.length > 1 || (turf.rewards_enabled && (turf.coins_per_booking ?? 0) > 0)) && (
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#E7E2DA] flex-wrap gap-1">
                {turf.sports.length > 1 && (
                  <div className="flex gap-1 flex-wrap">
                    {turf.sports.slice(1, 3).map((s) => (
                      <span
                        key={s}
                        className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full text-[10px] bg-[#F4F1EB] text-[#5F5F5F] border border-[#E7E2DA]"
                      >
                ...
                {turf.rewards_enabled && (turf.coins_per_booking ?? 0) > 0 && (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] bg-[#0D4D36]/8 text-[#0D4D36] border border-[#0D4D36]/15 font-medium">
